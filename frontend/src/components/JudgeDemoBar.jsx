...
export default function JudgeDemoBar({
  ...
  const stages = [
    { num: '01', title: t('stage1_title'), desc: t('stage1_desc') },
    { num: '02', title: t('stage2_title'), desc: t('stage2_desc') },
    { num: '03', title: t('stage3_title'), desc: t('stage3_desc') },
    { num: '04', title: t('stage4_title'), desc: t('stage4_desc') },
    { num: '05', title: t('stage5_title'), desc: t('stage5_desc') },
    { num: '06', title: t('stage6_title'), desc: t('stage6_desc') },
  ];
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-emerald-950 text-emerald-400 border border-emerald-600/70 px-1.5 py-0.2 rounded font-mono font-bold">
                {t('pipelineStage')} {currentStep} / {totalSteps}
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {onToggleAutoPlay && (
            <button
              ...
              title={autoPlay ? t('pause') : t('autoRun')}
            >
              {autoPlay ? (
                <>
                  <Pause className="w-3 h-3 text-amber-400" />
                  <span className="text-[11px]">{t('pause')}</span>
                </>
              ) : (
                <>
                  <Play className="w-3 h-3 text-emerald-400" />
                  <span className="text-[11px]">{t('autoRun')}</span>
                </>
              )}
            </button>
          )}
          ...
          {onNext && currentStep < totalSteps && (
            ...
            >
              <span>{isActionLoading ? t('processing') : nextLabel}</span>
          ...
          >
            ...
            <span className="text-[11px] hidden sm:inline">{t('exit')}</span>
          </button>
        </div>
      </div>
    </div>
